...
interface WindowsExporterProps {
  isOpen: boolean;
  onClose: () => void;
...
Wersja: 2.0 (Dual-Engine: PyQt5 + Tkinter Fallback / 100% Offline AI)
================================================================================
Instrukcja uruchomienia:
  1. Zainstaluj biblioteki:
     pip install PyQt5 keyboard pyautogui pyperclip deep-translator pytesseract pillow speechrecognition
...
const REQUIREMENTS_TXT = `PyQt5>=5.15.9
pyautogui>=0.9.54
keyboard>=0.13.5
pyperclip>=1.8.2
pytesseract>=0.3.10
...
title KursorAssist - Uruchamianie
echo ========================================================
echo   KursorAssist - Pływające Menu Ułatwień Dostępu
echo ========================================================
echo.
...
echo Sprawdzanie i instalacja bibliotek...
pip install -r requirements.txt

echo.
echo ========================================================
echo Aplikacja uruchomiona w tle!
echo Wcisnij skrot klawiszowy: Alt + Q
echo Okienko pojawi sie natychmiast przy Twoim kursorze myszy!
echo ========================================================
python kursor_assist.py
pause
`;

export const WindowsExporter: React.FC<WindowsExporterProps> = ({
  isOpen,
  onClose,
  soundEffects,
}) => {
  const [activeTab, setActiveTab] = useState<'script' | 'bat' | 'req'>('script');
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-3xl rounded-2xl bg-slate-900 border border-slate-700/80 shadow-2xl shadow-indigo-500/10 overflow-hidden text-slate-100 max-h-[90vh] flex flex-col"
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                Natywny Program na Windows (Python + Alt+Q)
              ...
              <p className="text-xs text-slate-400">
                Uruchamia pływające okienko pod kursorem myszy w całym systemie Windows
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 py-3 bg-slate-950/60 border-b border-slate-800 flex items-center justify-between text-xs">
          <div className="flex items-center gap-2 text-slate-300">
            ...
            <span>
              <b>Jak uruchomić na Windowsie:</b> 1. Pobierz pliki &nbsp;→&nbsp; 2. Kliknij dwukrotnie <span className="font-mono text-amber-400">start_kursor_assist.bat</span> &nbsp;→&nbsp; 3. Wciśnij <span className="font-mono text-blue-400 font-bold">Alt+Q</span>!
            </span>
          </div>
          <button
            onClick={() => {
              downloadFile('kursor_assist.py', PYTHON_SCRIPT_CODE);
              setTimeout(() => downloadFile('requirements.txt', REQUIREMENTS_TXT), 300);
              setTimeout(() => downloadFile('start_kursor_assist.bat', RUN_BAT), 600);
            }}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-md transition-colors shrink-0"
          >
            <Download className="w-3.5 h-3.5" />
            Pobierz całą paczkę (.zip / .py)
          </button>
        </div>
        ...
        <div className="p-6 flex-1 flex flex-col min-h-0 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <Download className="w-3.5 h-3.5" />
                Pobierz {currentFileName}
              </button>
            </div>
          </div>

          {/* Syntax Highlighted Box */}
          <div className="flex-1 min-h-[260px] max-h-[380px] rounded-xl bg-slate-950 border border-slate-800 overflow-hidden flex flex-col">
            <div className="px-4 py-2 bg-slate-900/60 border-b border-slate-800 text-[11px] font-mono text-slate-400 flex items-center justify-between">
              ...
              <span>100% Offline • Bez API</span>
            </div>
            <pre className="flex-1 p-4 text-xs font-mono text-slate-300 overflow-auto leading-relaxed selection:bg-indigo-500 selection:text-white">
